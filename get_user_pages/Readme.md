...
Launch tester, which allocates memory and maps it into the kernel.  
It then turns off preempt and locks the cpu, and constantly checks 
the page(s) for modifications (bytes turn from A to B).  

Once the change is detected, the driver the mutates the memory from
B to C.
